...
struct LongNumber power(struct LongNumber a, struct LongNumber b,struct LongNumber c)
{
	...
	struct LongNumber modres;
	modres = allocate(modres, res.size);
	modres=copy(a);

	while((ShortCompare(pow,0)) > 0)
	{
		if((pow.pointer[0] & 1)==1)
		{
			res = multiply(res, modres);
			res = module(res, c);
		}

		modres = multiply(modres, modres);
		modres = module(modres, c);
